...
#define GADAdidKey @"GADAdid" // info.plist里的广告单元ID
...
+ (instancetype)shared{
    ...
    dispatch_once(&onceToken, ^{
        if (nil == adext) {
            ...
            // 预加载广告
//            [adext loadAdid:InfoPlist(GADAdidKey) completionHandler:^(GADRewardedAd * _Nullable rewardedAd, NSError * _Nullable error) {}];
        }
    });
    return adext;
}
