...
@MainActor
@objcMembers
public final class VKIDBridge: NSObject {

    @objc public static let shared = VKIDBridge()

    private override init() {}
    private var vkid: VKID?

    public func config(clientId: NSString, clientSecret: NSString) {
        do {
            ...
            // 不建议 preconditionFailure，至少别在 release 里直接杀进程
            print("Failed to initialize VKID: \(error)")
            self.vkid = nil
        }
    }

    public func authorize(complete: @escaping (Int, String) -> Void) {
        ...
        Task { @MainActor in
            guard let rootVC = UIApplication.safeKeyWindowRootViewController else {
                complete(0, "")
                return
            }

            vkid.authorize(using: .uiViewController(rootVC)) { result in
                do {
                    ...
                    DispatchQueue.main.async { complete(0, "") }
                }
            }
        }
    }
    ...
    public func open(url: URL) -> Bool {
        guard let vkid = self.vkid else { return false }
        // 如果 VKID 的 open 返回 Bool：
        return vkid.open(url: url)
        // 如果 VKID 的 open 返回 Void：改成 vkid.open(url: url); return true
    }
}
